...
final class IndexCoordinatorTests: XCTestCase {
  func testReindexSkipsUnchangedAndProcessesChangedItems() async throws {
    ...
    let coordinator = IndexCoordinator(
      db: db, chunker: Chunker(targetChars: 32, overlapChars: 8), maxConcurrent: 2)
  ...
  func testProgressReportsDoneTotalAndCurrentItem() async throws {
    ...
    let items = [
      IngestItem(
        externalID: "1", displayName: "1.txt", fileType: .text, sizeBytes: 8,
        data: Data("alpha".utf8)),
      IngestItem(
        externalID: "2", displayName: "2.txt", fileType: .text, sizeBytes: 8,
        data: Data("beta".utf8)),
    ]
