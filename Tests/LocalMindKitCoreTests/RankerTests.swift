...
final class RankerTests: XCTestCase {
  ...
  func testRecencyDecayPrefersRecentDates() {
    let now = Date()
    let recent = Ranker.recency(
      modifiedAt: now.addingTimeInterval(-86_400), now: now, halfLifeDays: 90)
    let old = Ranker.recency(
      modifiedAt: now.addingTimeInterval(-200 * 86_400), now: now, halfLifeDays: 90)
