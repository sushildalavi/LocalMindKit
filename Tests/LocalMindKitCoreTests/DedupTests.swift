...
final class DedupTests: XCTestCase {
  func testReUpsertSameExternalIDReplacesChunksWithoutOrphans() async throws {
    ...
    try await db.insertChunks([
      Chunk(
        fileID: fileID1, ordinal: 0, text: "old chunk one", charStart: 0, charEnd: 12,
        source: .plaintext),
      Chunk(
        fileID: fileID1, ordinal: 1, text: "old chunk two", charStart: 13, charEnd: 25,
        source: .plaintext),
    ])
    ...
    try await db.insertChunks([
      Chunk(
        fileID: fileID2, ordinal: 0, text: "new content", charStart: 0, charEnd: 10,
        source: .plaintext)
    ])
