...
final class IndexingViewModel {
  ...
  func startMockRun() {
    ...
    let fixtureItems = [
      IngestItem(
        externalID: "fixture-1", displayName: "Notes.txt", fileType: .text, sizeBytes: 58,
        data: Data("apple application tracker notes and deadlines".utf8)),
      IngestItem(
        externalID: "fixture-2", displayName: "Roadmap.md", fileType: .text, sizeBytes: 64,
        data: Data("local indexing, sqlite fts, vision ocr progress".utf8)),
    ]
  ...
  func ingestScreenshots(limit: Int = 150) {
    ...
    activeTask = Task {
      ...
      do {
        ...
        state = .complete
        summaryText =
          "Screenshots indexed: \(summary.indexed), skipped: \(summary.skipped), failed: \(summary.failed)."
      } catch is CancellationError {
        state = .cancelled
        summaryText = "Screenshot indexing cancelled."
      } catch {
